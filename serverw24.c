...
#define MIRROR1_PORT 9231
#define MIRROR2_PORT 9232
...
void redirectConnection(int clientSocket, int targetPort, const char *serverName) {
    printf("Redirecting connection to %s on port %d\n", serverName, targetPort);
    ...
    forwardAddr.sin_port = htons(targetPort);
    forwardAddr.sin_addr.s_addr = inet_addr(SERVER_IP);

     if (connect(forwardSocket, (struct sockaddr *)&forwardAddr, sizeof(forwardAddr)) < 0) {
        perror("Connect to mirror failed");

        // Send a custom error message back to the client before closing the sockets
        const char *errorMessage = "Failed to connect to mirror server.\n";
    ...
    while (bytesReceived > 0) {
        send(forwardSocket, buffer, bytesReceived, 0);  // Forward client's message to the mirror
        bytesReceived = recv(forwardSocket, buffer, BUFFER_SIZE, 0);  // Get response from the mirror
        if (bytesReceived > 0) {
            send(clientSocket, buffer, bytesReceived, 0);  // Send response back to the client
        }
        bytesReceived = recv(clientSocket, buffer, BUFFER_SIZE, 0);  // Continue receiving from client
    }
...
int main() {
    ...
    while (1) {
        ...
        printf("Connection %d accepted from %s:%d\n", connection_count, inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
        ...
        if (pid == 0) { // Child process
            ...
            if (connection_count <= 3) {
                ...
                redirectConnection(clientSocket, MIRROR1_PORT, "mirror1");
            } else if (connection_count <= 9) {
                printf("Connection %d handled by mirror2\n", connection_count);
                redirectConnection(clientSocket, MIRROR2_PORT, "mirror2");
            } else {
                int mod_count = (connection_count - 10) % 3 + 1;
                if (mod_count == 1) {
                    printf("Connection %d handled by serverw24\n", connection_count);
                    handleClient(clientSocket);
                } else if (mod_count == 2) {
